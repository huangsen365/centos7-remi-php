current_dir=$(pwd)
httpd_dir_name=httpd_$(date +%Y-%m-%d_%H%M%S)_$(hostname)
cp -pr $current_dir/volumes/volume_yourdomain.com/var/log/httpd $current_dir/volumes/volume_yourdomain.com/var/log/$httpd_dir_name
...
docker run -d \
  ...
  -v $current_dir/volumes/volume_yourdomain.com/var/log/$httpd_dir_name:/var/log/httpd \
  ...
  -p 2222:22 \
  --restart always \
  --name centos7-remi-php-docker_yourdomain.com_$httpd_dir_name \
  centos7-remi-php
